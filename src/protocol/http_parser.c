                                  ...
                                  http_parser_t *parser)
{
    ...
    do
    {
        ...
        if (parser->header_state == HPS_START_LINE)
            ...
            ret = __parser_header_value(ptr, len, parser);
        else
        {
            ret = __parser_header_name(ptr, len, parser);
                         ...
                         http_parser_t *parser)
{
    ...
    do
    {
        ...
        if (parser->chunk_state == CPS_CHUNK_DATA)
            ret = __parser_chunk_data(ptr, len, parser);
        else
        {
            ret = __parser_trailer_part(ptr, len, parser);
                               ...
                               http_parser_t *parser)
{
    ...
    if (parser->header_state != HPS_HEADER_COMPLETE)
    {
        ret = __parser_message_header(parser->msgbuf, parser->msgsize, parser);
        if (ret <= 0)
            return ret;
    ...
    if (parser->chunk_state != CPS_CHUNK_COMPLETE)
    {
        ret = __parser_chunk(parser->msgbuf, parser->msgsize, parser);
        if (ret <= 0)
            return ret;
    }
...
int http_parser_add_header(const void *name, size_t name_len,
                           ...
                           http_parser_t *parser)
{
    if (__add_message_header(name, name_len, value, value_len, parser))
    {
